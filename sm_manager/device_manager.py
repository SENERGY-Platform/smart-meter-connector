...
class DeviceManager:
    ...
    def __init__(self):
        ...
        with sqlliteConnect(self.__db_path) as conn:
            conn.execute("CREATE TABLE IF NOT EXISTS devices (id TEXT PRIMARY KEY UNIQUE, type TEXT)")
        conn.close()
        self.__loadFromDB()
    ...
    def add(self, device: cc_lib.types.Device, d_type) -> None:
        ...
        if device.id not in self.__device_pool:
            ...
            with sqlliteConnect(self.__db_path) as conn:
                conn.execute("INSERT INTO devices (id, type) VALUES (?, ?)", (device.id, d_type))
            conn.close()
        else:
    ...
    def __loadFromDB(self):
        with sqlliteConnect(self.__db_path) as conn:
            for id, type in conn.execute("SELECT * FROM devices"):
                self.__device_pool[id] = device_type_map[type](id, None)
        conn.close()
